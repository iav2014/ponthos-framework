...
let redisLink0 = require('redis');
let redisPool = require('./pool');

function redisConnector(config, callback) {
	let storeDB = new createClient(redisLink0, config.host, config.port,config.attempt);
	storeDB.select(config.database, function (err, res) {
		if (err) {
			...
			let uri=`${config.host}:${config.port}:${config.database}`;
	...
	storeDB.on('error', function (err) {
		...
		let uri=`${config.host}:${config.host}:${config.database}`;
